...
const drawLogitLayer = (arg) => {
  ...
  logitLayer.append('circle')
    ...
    .attr('cx', centerX)
    .attr('cy', nodeCoordinate[curLayerIndex][selectedI].y + nodeLength / 2)
    .attr('r', logitRadius)
  ...
  logitLayer.append('line')
    ...
    .attr('x2', softmaxX)
    .attr('y1', nodeCoordinate[curLayerIndex][selectedI].y + nodeLength / 2)
    .attr('y2', nodeCoordinate[curLayerIndex][selectedI].y + nodeLength / 2)
    .style('fill', 'none')
    .style('stroke', '#EAEAEA')
    .style('stroke-width', '1.2')
    .lower();
  ...
  const drawOneEdgeGroup = () => {
    ...
    if (curEdgeGroup.empty()) {
      ...
      for (let vi = 0; vi < cnn[curLayerIndex-1].length; vi++) {
        linkData.push({
          source: {x: intermediateX1 + 1.5 * nodeLength - moveX,
            y: nodeCoordinate[curLayerIndex-1][vi].y + nodeLength / 2},
          target: {x: intermediateX2 - moveX,
            y: nodeCoordinate[curLayerIndex][curI].y + nodeLength / 2},
          index: -1,
          color: '#EDEDED',
    ...
    curNodeGroup.append('circle')
      ...
      .attr('cx', centerX)
      .attr('cy', nodeCoordinate[curLayerIndex][curI].y + nodeLength / 2)
      .attr('r', 7)
...
const softmaxClicked = (arg) => {
  ...
  for (let i = curLayerIndex - 1; i >= 0; i--) {
    ...
    let previousX;
    if(i === 5){
      previousX = +curLayer.select('circle.dense-circle').attr('cx') - nodeLength / 2;
    }else{
    ...
    .on('end', () => {
      if (!isInSoftmax) {
        ...
        for (let i = 0; i < 15; i++) {
          softmaxDetailAnnotation.append('text')
            .attr('x', centerX)
            .attr('y', nodeCoordinate[curLayerIndex][i].y + nodeLength / 2 + 8)
            .attr('class', 'annotation-text softmax-detail-text')
        ...
        textX = centerX + 50;
        textY = nodeCoordinate[curLayerIndex][0].y + nodeLength / 2;

        if (selectedI < 3) {
          textY = nodeCoordinate[curLayerIndex][14].y + nodeLength / 2;
        }
 ...
 */
export const drawOutFlatten = (curLayerIndex, d, i, width, height) => {
  ...
  let intermediateGap = totalLength / 2 - 3 * nodeLength;
  const minimumGap = 20;
  let linkGen = d3.linkHorizontal()
    .x(d => d.x)
    .y(d => d.y);
  ...
  for (let n = 0; n < cnn[curLayerIndex - 1].length; n++) {
      Ys.push(nodeCoordinate[curLayerIndex - 1][n].y + nodeLength / 2);
  }
  ...
  let flattenExtent = d3.extent(cnn[curLayerIndex-1].slice(0)
    .map(d => d.outputLinks[i].weight)
    .concat(cnn[curLayerIndex-1].slice(9)
      .map(d => d.outputLinks[i].weight)));
  ...
  for (let f = 0; f < 10; f++) {
      let factoredF = f;
      flattenFactoredFDict[factoredF] = cnn[curLayerIndex-1][factoredF].output;
      ...
      linkData.push({
        ...
        source: {x: leftX + nodeLength + 3,
          y: nodeCoordinate[curLayerIndex - 1][f].y + nodeLength / 2},
        index: factoredF,
        weight: cnn[curLayerIndex-1][factoredF].outputLinks[i].weight,
        name: `flatten-${factoredF}-output`,
        color: gappedColorScale(layerColorScales.weight,
          flattenRange, cnn[curLayerIndex-1][factoredF].outputLinks[i].weight, 0.35),
        width: 1,
        opacity: 1,
  ...
  let middleGap = 5;
  let middleRectHeight = (10 * nodeLength + (10 - 1) * vSpaceAroundGap - 5 * (8 + 1)) / 8;
  ...
  drawIntermediateLayerLegend({
    ...
    curLayerIndex: curLayerIndex,
    range: range,
    minMax: cnnLayerMinMax[min_layer_index],
  ...
  outputAnnotation.append('text')
    .attr('x', nodeCoordinate[curLayerIndex][i].x)
    .attr('y', nodeCoordinate[curLayerIndex][i].y + 10)
    .attr('class', 'annotation-text')
    .text(`(${d3.format('.4f')(cnn[curLayerIndex][i].output)})`);
...
export const drawFlatten = (curLayerIndex, d, i, width, height) => {
  ...
  let pixelHeight = 1.1;
  let totalLength = (2 * nodeLength +
    2.5 * hSpaceAroundGap * gapRatio + pixelWidth);
  let leftX = nodeCoordinate[curLayerIndex][0].x - totalLength;
  // let intermediateGap = (hSpaceAroundGap * gapRatio * 4) / 2;
  let intermediateGap = totalLength / 2 - 3 * nodeLength;
  const minimumGap = 20;
  ...
  intermediateLayerOverlay.append('rect')
    ...
    .style('fill', 'url(#overlay-gradient-left)')
    .style('stroke', 'none')
    .attr('width', leftX + svgPaddings.left - (leftGap * 2) + 5)
    .attr('height', height + svgPaddings.top + svgPaddings.bottom)
    .attr('x', -svgPaddings.left)
    .attr('y', 0)
    .style('opacity', 0);
  ...
  intermediateLayerOverlay.append('rect')
    ...
    .attr('height', height + svgPaddings.top + svgPaddings.bottom)
    .attr('x', nodeCoordinate[curLayerIndex][0].x + nodeLength + 5)
    .attr('y', 0)
    .style('opacity', 0);
  ...
  let Ys = [];
  for (let n = 0; n < cnn[curLayerIndex - 1].length; n++) {
    if(n < cnn[curLayerIndex - 1].length / 2){
      Ys.push(nodeCoordinate[curLayerIndex - 1][n].y);
    }else{
      Ys.push(nodeCoordinate[curLayerIndex - 1][n].y + nodeLength - flattenLength * pixelHeight);
    }
  }
