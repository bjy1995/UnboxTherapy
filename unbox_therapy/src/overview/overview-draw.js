...
const drawLegends = (legends, legendHeight) => {
  ...
  for (let i = 0; i < 3; i++){
    ...
    let range2 = cnnLayerRanges.local[start + 1];

    let localLegendScale1 = d3.scaleLinear()
      .range([0, 1 * nodeLength + 0 * hSpaceAroundGap - 1.2])
      .domain([-range1 / 2, range1 / 2]);
    
    let localLegendScale2 = d3.scaleLinear()
      .range([0, 2 * nodeLength + 1 * hSpaceAroundGap - 1.2])
    ...
    let localLegend1 = legends.append('g')
      ...
      .attr('transform', `translate(${nodeCoordinate[start][0].x}, ${svgPaddings.bottom})`);
    ...
    localLegend1.append('rect')
      .attr('width', 1 * nodeLength + 0 * hSpaceAroundGap)
    ...
    let localLegend2 = legends.append('g')
      ...
      .classed('hidden', !detailedMode || selectedScaleLevel !== 'local')
      .attr('transform', `translate(${nodeCoordinate[start + 1][0].x}, ${svgPaddings.bottom})`);
    ...
    localLegend2.append('rect')
      .attr('width', 2 * nodeLength + 1 * hSpaceAroundGap)
      .attr('height', legendHeight)
      .style('fill', 'url(#convGradient)');
  }
  ...
  for (let i = 0; i < 3; i++){
    ...
    let moduleLegendScale = d3.scaleLinear()
      .range([0, 2 * nodeLength + 1 * hSpaceAroundGap +
  ...
  let globalLegendScale = d3.scaleLinear()
    .range([0, 7 * nodeLength + 6 * hSpaceAroundGap +
      4 * hSpaceAroundGap * gapRatio - 1.2])
    .domain([-range / 2, range / 2]);
  ...
  globalLegend.append('rect')
    .attr('width', 7 * nodeLength + 6 * hSpaceAroundGap +
      4 * hSpaceAroundGap * gapRatio)
    .attr('height', legendHeight)
    .style('fill', 'url(#convGradient)');
  ...
  let outputLegend = legends.append('g')
    ...
    .attr('transform', `translate(${nodeCoordinate[6][0].x}, ${svgPaddings.bottom})`);
  ...
  let inputLegend = legends.append('g')
    ...
    .classed('hidden', !detailedMode)
    .attr('transform', `translate(${nodeCoordinate[0][0].x}, ${svgPaddings.bottom})`);
...
export const drawCNN = (width, height, cnnGroup, nodeMouseOverHandler,
  ...
  for (let l = 0; l < cnn.length; l++) {
    ...
    if (curLayer[0].layerName === 'output') {
        ...
        .attr('cx', left + nodeLength / 2)
        .attr('cy', (d, i) => nodeCoordinate[l][i].y + nodeLength / 2)
        .attr('r', nodeLength / 2)
